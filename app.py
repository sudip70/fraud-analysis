...
# ── Page config ────────────────────────────────────────────────────────────────
st.set_page_config(page_title="FraudShield", page_icon="🛡️", layout="wide", initial_sidebar_state="collapsed")
...
<style>
  ...
  .kpi-card {{
    background: linear-gradient(135deg, {CARD} 0%, #0F1E3A 100%);
  ...
  div[data-testid="metric-container"] {{ background:{CARD}; }}
...
def neon_line(ax, x, y, color, lw=2.2, glow=True, label=None, marker=None):
    ...
    if glow:
        ax.plot(x, y, color=color, linewidth=lw+4, alpha=0.15, solid_capstyle="round")
        ax.plot(x, y, color=color, linewidth=lw+1.5, alpha=0.3, solid_capstyle="round")
...
def neon_area(ax, x, y, color, alpha_fill=0.25, lw=2.0, label=None, baseline=0):
    """Draw area chart with neon glow fill."""
...
def neon_bar_h(ax, categories, values, color_start, color_end, annotate=True):
    ...
    bars = ax.barh(categories, values, color=colors, height=0.55, zorder=3, edgecolor="none")
    # Glow shadow
    ax.barh(categories, values, color=colors, height=0.65, alpha=0.15, zorder=2, edgecolor="none")
    if annotate:
        for bar, v in zip(bars, values):
            ax.text(bar.get_width() + max(values)*0.01,
                    bar.get_y() + bar.get_height()/2,
                    f"{v:.2%}", va="center", fontsize=8.5,
...
<div style="padding:36px 0 16px">
  <div style="font-size:11px;font-weight:600;color:{NEON["cyan"]};letter-spacing:.1em;text-transform:uppercase;margin-bottom:12px">
  ...
  <p style="color:{MUTED};font-size:15px;font-weight:300;margin-top:12px;max-width:560px;line-height:1.7">
    A full-stack fraud detection system — EDA insights, ML model comparison, feature importance, and a live transaction scorer.
  </p>
</div>
<hr>
""", unsafe_allow_html=True)
...
for col,(val,lbl,sub) in zip([c1,c2,c3,c4,c5],cards):
    sh = f'<div class="kpi-sub">{sub}</div>' if sub else ""
    col.markdown(f'<div class="kpi-card"><div class="kpi-val">{val}</div><div class="kpi-lbl">{lbl}</div>{sh}</div>',
                 unsafe_allow_html=True)

st.markdown("<br>", unsafe_allow_html=True)
tab1, tab2, tab3 = st.tabs(["📊  Exploratory Analysis", "🤖  Model Performance", "🎯  Live Fraud Scorer"])
...
# ══════════════════════════════════════════════════════════════════════════════
with tab1:
    st.markdown(f"### <span style='color:{TEXT}'>Fraud Patterns</span> <span style='color:{NEON["cyan"]}'>at a Glance</span>", unsafe_allow_html=True)
    ...
    with col1:
        ...
        neon_bar_h(ax, data["Transaction_Type"], data["fraud_rate"],
                   NEON["blue"], NEON["cyan"])
        ax.set_title("Fraud Rate by Transaction Type", fontsize=11, fontweight="bold",
    ...
    with col2:
        ...
        ax.set_title("Fraud Rate by Merchant Category", fontsize=11, fontweight="bold",
                     color=TEXT, pad=12, loc="left")
        fig.tight_layout(pad=1.5)
        st.pyplot(fig, use_container_width=True); plt.close()
    ...
    with col3:
        ...
        norm = plt.Normalize(data["fraud_rate"].min(), data["fraud_rate"].max())
        cmap = LinearSegmentedColormap.from_list("city", [NEON["blue"], NEON["cyan"]])
        cols = [cmap(norm(v)) for v in data["fraud_rate"]]
        bars = ax.barh(data["Transaction_Location"], data["fraud_rate"],
                       color=cols, height=0.58, zorder=3, edgecolor="none")
        ax.barh(data["Transaction_Location"], data["fraud_rate"],
                color=cols, height=0.68, alpha=0.12, zorder=2, edgecolor="none")
        ax.xaxis.set_major_formatter(mtick.PercentFormatter(xmax=1, decimals=1))
        ax.set_title("Fraud Rate by City", fontsize=11, fontweight="bold",
                     color=TEXT, pad=12, loc="left")
        ax.grid(axis="x", color=GRID, linewidth=0.7)
        ax.grid(axis="y", visible=False)
    ...
    with col4:
        ...
        fig, ax = dark_fig(5.5, 3.8)
        x = data["Hour"].values
        y = data["fraud_rate"].values
        neon_area(ax, x, y, NEON["cyan"], alpha_fill=0.3, lw=2.2)
        peak_idx = np.argmax(y)
        ax.scatter(x[peak_idx], y[peak_idx], color=NEON["cyan"], s=80, zorder=10,
                   edgecolors=BG2, linewidths=1.5)
        annotation_box(ax, x[peak_idx], y[peak_idx],
                       f"{y[peak_idx]:.1%}", NEON["cyan"])
        ax.yaxis.set_major_formatter(mtick.PercentFormatter(xmax=1, decimals=1))
        ax.set_xlabel("Hour of Day", fontsize=9)
        ax.set_xlim(0, 23)
        ax.set_title("Fraud Rate by Hour of Day", fontsize=11, fontweight="bold",
                     color=TEXT, pad=12, loc="left")
        fig.tight_layout(pad=1.5)
        st.pyplot(fig, use_container_width=True); plt.close()
    ...
    with col5:
        ...
        n_vals, _, patches_n = ax.hist(eda["amount_normal"], bins=bins,
                                       alpha=0, density=True)
        f_vals, _, patches_f = ax.hist(eda["amount_fraud"], bins=bins,
                                       alpha=0, density=True)
        bin_centers = (bins[:-1] + bins[1:]) / 2
        neon_area(ax, bin_centers, n_vals, NEON["green"], alpha_fill=0.25, lw=2.0, label="Normal")
        neon_area(ax, bin_centers, f_vals, NEON["red"],   alpha_fill=0.3,  lw=2.0, label="Fraud")
    ...
    with col6:
        ...
        fig, ax = dark_fig(5.5, 3.2)
        x_labels = prev["Previous_Fraud_Count"].astype(str).tolist()
        ...
        bars = ax.bar(x_labels, rates, color=cols2, width=0.5, zorder=3, edgecolor="none")
        ax.bar(x_labels, rates, color=cols2, width=0.6, alpha=0.15, zorder=2, edgecolor="none")
        ax.yaxis.set_major_formatter(mtick.PercentFormatter(xmax=1, decimals=1))
    ...
    # ── Row 4: Risk flags + Distance violin ───────────────────────────────────
    col7, col8 = st.columns(2)

    with col7:
        intl = pd.DataFrame(eda["fraud_by_international"])
        newm = pd.DataFrame(eda["fraud_by_new_merchant"])
        fig, axes = dark_fig(5.5, 2.8, ncols=2)
        ax1, ax2 = axes
        for ax_i, (d, title, c_lo, c_hi) in zip([ax1,ax2], [
            (intl, "International Txn", NEON["blue"],   NEON["cyan"]),
            (newm, "New Merchant",      NEON["purple"], NEON["pink"]),
        ]):
            rates_d = d["fraud_rate"].values
            labs = d.iloc[:,0].astype(str).tolist()
            cmap_i = LinearSegmentedColormap.from_list("ri", [c_lo, c_hi])
            colors_i = [cmap_i(i/max(len(rates_d)-1,1)) for i in range(len(rates_d))]
            bars_i = ax_i.bar(labs, rates_d, color=colors_i, width=0.45, zorder=3, edgecolor="none")
            ax_i.bar(labs, rates_d, color=colors_i, width=0.55, alpha=0.15, zorder=2, edgecolor="none")
            ax_i.yaxis.set_major_formatter(mtick.PercentFormatter(xmax=1, decimals=1))
            ax_i.set_title(title, fontsize=10, fontweight="bold", color=TEXT, pad=8)
            for bar in bars_i:
                ax_i.text(bar.get_x()+bar.get_width()/2, bar.get_height()+0.001,
                          f"{bar.get_height():.2%}", ha="center", fontsize=8.5,
                          color=TEXT, fontweight="500")
        fig.suptitle("Fraud Rate by Risk Flags", fontsize=11, fontweight="bold",
                     color=TEXT, y=1.02, x=0.05, ha="left")
        fig.tight_layout(pad=1.5)
        st.pyplot(fig, use_container_width=True); plt.close()

    with col8:
        norm_d  = np.array(eda["distance_normal"])
        fraud_d = np.array(eda["distance_fraud"])
        fig, ax = dark_fig(5.5, 2.8)
        vp = ax.violinplot([norm_d, fraud_d], positions=[0,1],
    ...
    # ── Insight cards ─────────────────────────────────────────────────────────
    st.markdown("---")
    st.markdown(f"#### <span style='color:{TEXT}'>💡 Key Insights</span>", unsafe_allow_html=True)
    ins_cols = st.columns(3)
    insights = [
        (NEON["cyan"],   "🌍", "International + New Merchant",
         "Transactions flagged as both international AND at a new merchant carry the highest fraud rate — the strongest combined signal."),
        (NEON["purple"], "🕐", "Late-Night Spike",
         "Fraud peaks in late-night hours. Unusual_Time_Transaction is one of the top predictors across all three trained models."),
        (NEON["pink"],   "📍", "Distance from Home",
         "Fraudulent transactions occur on average 2× farther from the customer's home city — a strong standalone risk indicator."),
    ]
    for col, (accent, icon, title, body) in zip(ins_cols, insights):
        col.markdown(
            f"""<div style="background:{CARD};border:1px solid {GRID};border-left:3px solid {accent};
...
# ══════════════════════════════════════════════════════════════════════════════
with tab2:
    st.markdown(f"### <span style='color:{TEXT}'>Model Comparison</span> <span style='color:{NEON["purple"]}'>& Evaluation</span>", unsafe_allow_html=True)
    st.caption("Three classifiers trained with class balancing to handle the 4.8% fraud imbalance.")
    st.markdown("")
    ...
    with col1:
        ...
        ax.set_title("ROC Curve", fontsize=11, fontweight="bold", color=TEXT, pad=12, loc="left")
    ...
    with col3:
        ...
        cm = np.array(results[arts["best_name"]]["cm"])
        ...
        for i in range(2):
            for j in range(2):
                clr = BG2 if cm[i,j] > max_v*0.5 else TEXT
    ...
    with col4:
        ...
        fi = arts["feature_importance"].head(12).copy().sort_values("importance")
        fig, ax = dark_fig(4.8, 3.8)
        cmap_fi = LinearSegmentedColormap.from_list("fi", [NEON["purple"], NEON["cyan"]])
        n = len(fi)
        cols_fi = [cmap_fi(i/max(n-1,1)) for i in range(n)]
        ...
        fig.tight_layout(pad=1.5); st.pyplot(fig, use_container_width=True); plt.close()
...
# ══════════════════════════════════════════════════════════════════════════════
with tab3:
    st.markdown(f"### <span style='color:{TEXT}'>Live</span> <span style='color:{NEON["green"]}'>Fraud Scorer</span>", unsafe_allow_html=True)
    st.caption("Enter a transaction and get an instant fraud probability with a plain-English risk breakdown.")
    st.markdown("")
    ...
    with col_form:
        with st.form("scorer_form"):
            ...
            tx_location  = fc2.selectbox("Transaction Location",["Karachi","Lahore","Islamabad","Faisalabad","Multan","Dubai","London","Singapore","Bangkok","Kuala Lumpur"])
            home_loc     = fc1.selectbox("Home Location",     ["Karachi","Lahore","Islamabad","Faisalabad","Multan","Dubai","London","Singapore","Bangkok","Kuala Lumpur"])
    ...
    with col_result:
        if submitted:
            row = pd.DataFrame([{
                "Transaction_Amount (in Million)":amount,"Transaction_Time":tx_time,
                "Transaction_Date":"2025-01-01","Transaction_Type":tx_type,
                "Merchant_Category":merchant_cat,"Transaction_Location":tx_location,
                "Customer_Home_Location":home_loc,"Distance_From_Home":distance,
                "Card_Type":card_type,"Account_Balance (in Million)":balance,
                "Daily_Transaction_Count":daily_tx,"Weekly_Transaction_Count":weekly_tx,
                "Avg_Transaction_Amount (in Million)":avg_amount,
                "Max_Transaction_Last_24h (in Million)":max_24h,
                "Is_International_Transaction":is_intl,"Is_New_Merchant":is_new,
                "Failed_Transaction_Count":failed,"Unusual_Time_Transaction":unusual,
            ...
            X_input = preprocess(row, encoders=arts["encoders"], fit=False)
            X_input = X_input.reindex(columns=arts["feature_names"], fill_value=0)
            prob    = arts["best_model"].predict_proba(X_input)[0][1]

            if prob>=0.60:   tier,tier_class,color,gcolor="HIGH","risk-high",NEON["red"],"#1F0A0F"
            elif prob>=0.30: tier,tier_class,color,gcolor="MEDIUM","risk-medium",NEON["yellow"],"#1F180A"
            else:            tier,tier_class,color,gcolor="LOW","risk-low",NEON["green"],"#0A1F14"

            st.markdown(f"""
            <div style="background:{CARD};border:1px solid {GRID};border-radius:14px;
                        padding:28px 24px;margin-top:4px;position:relative;overflow:hidden;">
            ...
            ax.barh([0], [prob], color=color, height=0.55, alpha=0.2, zorder=1)
            ax.set_xlim(0,1); ax.axis("off")
            plt.tight_layout(pad=0)
            st.pyplot(fig, use_container_width=True); plt.close()

            st.markdown(f"<div style='color:{MUTED};font-size:12px;margin-bottom:12px'>{prob:.1%} probability · {tier} risk tier</div>", unsafe_allow_html=True)
            st.markdown(f"**<span style='color:{TEXT}'>Why was this flagged?</span>**", unsafe_allow_html=True)
            ...
            if unusual=="Yes":         flags.append(f"🕐 **Unusual hour** — outside normal window")
            ...
            st.markdown(f"<div style='margin-top:16px;font-size:11px;color:{MUTED}'>Model: {arts['best_name']} · ROC-AUC {arts['model_results'][arts['best_name']]['roc_auc']:.4f}</div>",unsafe_allow_html=True)
        else:
            st.markdown(f"""
            <div style="background:{CARD};border:1px dashed {GRID};border-radius:14px;
              ...
              <div style="font-size:13px;color:{MUTED};line-height:1.7">
                Fill in the transaction details on the left<br>and click <strong>Analyse Transaction</strong>
              </div>
            </div>""", unsafe_allow_html=True)

st.markdown("---")
st.markdown(f"<div style='text-align:center;font-size:12px;color:{MUTED};padding:8px 0 28px'>FraudShield · Python · scikit-learn · Streamlit · 50,000 transactions</div>", unsafe_allow_html=True)